...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[->,>=stealth',level/.style={sibling distance = 7cm/#1,
      ...
      \node {Strategies for improving local search}
        ...
        child { node [yshift=-1.2 cm] {Change landscape\\of the problem}
            child { node [xshift=-0.8 cm] {Change of the objective function\\or the input data}
                child { node {Guided\\local\\search} }
                child { node {Noisy method} }
                child { node {Smoothing\\method} } }
            child { node [xshift=0.8 cm]  {Use different\\neighbourhoods}
                child { node {Variable\\neighbourhood\\search} }
            } 
        }
    ...
    \caption{Single-solution metaheuristics as specialized version of local search. \todo{source}}
...
The noisy method is a another metaheuristic, which changes the fitness space to escape local optima. It is done by adding a random noise to the input data [metaheuristics book, pp 160]. At the beginning of the computation, the noise is the strongest, and it disappears during the computation, so the final \textit{epochs} can be evaluated on the original data.
...
\begin{enumerate}
 ...
 \item Finally, the table generator script generates a human readable version of the results as well as database dump of the whole experiment \cref{fig:scripts}.
\end{enumerate}
