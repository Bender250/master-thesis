...
\thesislong{abstract}{%
Statistical testing of randomness is one of the automated ways of cryptanalysis of cryptoprimitives. While traditional statistical batteries use fixed tests, research tool EACirc developed at Faculty of Informatics, Masaryk University designs the tests dynamically to the analysed data. Its goal is to distinguish tested data from random data. The tool internally utilises simple heuristic based on hill climbing. Moreover, this thesis researches other metaheuristics and their potential application in EACirc tool, as well as applying classification models as artificial neural networks.\\\\%
%
The thesis has the following contributions:
\begin{itemize}
    \item Development of a testbed of 16 well-known cryptographic functions with necessary tools for direct comparison of various research approaches tested on same data. The generator of the dataset as well as the data itself are published online, and our results are compared to randomness testing tools using these data.
    \item Extension of EACirc by three new metaheuristics. One of them outperforms all the others in terms of its success rate, and it also provides a further advantage of more practical cryptanalysis.
