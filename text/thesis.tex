...
\begin{itemize}
    \item GLS is always more successful than ITS and other methods (SA has worse reference value due to connection bug).
    \item ANN can be bit-oriented, but we have not seen any advantage of it, even thou it is crucial capability for crypto data
    \item Advantage of these single-solution metaheuristis (ANN is also using just one solution in parallel) has much simpler interpretation, as we do not care about dependency of the statistic result
    \item 
\end{itemize}
...
\begin{itemize}
    \item GLS is always more successful than ITS
    \item GLS produces simpler circuit, with addition of pruning, this decreases significantly complexity of cryptoanalysis
    \item ANN/CNN can be used only as very lightweight tests, with very low data requirements and low success rate
    \item TestU01 is far better than EACirc
    \item No free lunch theorem
    \item We found suprisingly errors in full round Rabbit
\end{itemize}
